...
type MarkdownRendererProps = {
  ...
  textScale: TextScale;
  darkMode?: boolean;
  /** Lets the maintained renderer expose native/browser text selection. */
  selectable?: boolean;
  /** Keeps prose readable while tables and diagrams use the available width. */
  wideLayout?: boolean;
  availableWidth?: number;
  /** Persists the renderer's interactive GFM task-list changes in Marden. */
...
const splitMarkdown = (content: string, separateWideTables: boolean): RenderSegment[] => {
  ...
  const pushMarkdown = (markdown: string, fullWidth: boolean) => {
    ...
    segments.push({ kind: 'markdown', markdown, fullWidth, taskOffset });
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (fenceMatch) {
      ...
      if (language === 'mermaid') {
        ...
        segments.push({ kind: 'mermaid', source: diagramLines.join('\n') });
        continue;
      }
