...
Marden is an Expo/React Native iPhone app, not a web wrapper. It can import from the iOS Files picker, persist Markdown in its private app storage, render Markdown and Mermaid diagrams, and use the reader's saved light or dark theme.

There are three useful ways to run it on an iPhone. Choose Route 1 for the fastest test, Route 2 for a free standalone development install, or Route 3 for the smoothest self-contained sideload.

## Route 1: test immediately with Expo Go

This is the quickest option and does not need Xcode, an Expo account, or Apple signing. It runs Marden inside Expo Go, so it is not a separate Marden home-screen app.

1. Install **Expo Go** from the App Store on the iPhone.
2. Put the Mac and iPhone on the same Wi-Fi network.
3. In Terminal, run:
...
5. If the phone cannot reach the Mac over the local network, stop Metro with `Control-C`, then use:

   ```bash
   npx expo start --go --tunnel
   ```

Keep the Metro terminal running while using the app.

## Route 2: install directly with Xcode and a free Apple Account
...
1. Install the full **Xcode** app from the Mac App Store. Open it once and let it install the requested iOS components.
...
5. If Xcode says the bundle identifier is unavailable, replace `app.marden.reader` with a unique value such as `com.yourname.marden` in the Bundle Identifier field.
6. Choose the connected iPhone as the run destination in Xcode's top toolbar.
7. In a separate Terminal window, start Metro from the project folder and leave it running:
...
## Route 3: self-contained EAS internal build
...
## Optional: development-client EAS build

Use this instead of `preview` when actively changing code and wanting fast refresh:
...
| --- | --- | --- | --- |
| Try Marden in minutes | Expo Go | No | Yes |
